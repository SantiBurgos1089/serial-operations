...
from serialoper.common.settings_app import GeneralSettings

# Variable global para detener correctamente y el manejo del Websocket
running = True
...
# Esta funciona se ejecuta cuando systemd envia una señal de apagado (SIGINT o SIGTERM) 
# para detener el servicio de forma ordenada.
def signal_handler(sig, frame):
    global running
    print("[Headless] Recibida señal de apagado. Deteniendo operaciones...")
    running = False

# Captura señales de systemd (stop/restart)
signal.signal(signal.SIGINT, signal_handler) # Captura Ctrl+C (SIGINT)
signal.signal(signal.SIGTERM, signal_handler) # Captura el "stop" enviado por systemd
